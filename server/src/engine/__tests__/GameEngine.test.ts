...
function getPlayerByRole(engine: GameEngine, players: PlayerInfo[], role: string): PlayerState | undefined {
...
function getSolution(engine: GameEngine, players: PlayerInfo[]): { meansCardId: string; clueCardId: string } {
...
describe('GameEngine', () => {
  ...
  describe('夜晚阶段 - 凶手选牌', () => {
    ...
    it('should accept valid means + clue selection from murderer', () => {
      const murderer = getPlayerByRole(engine, players, 'murderer');
      expect(murderer).toBeDefined();
      const result = engine.handleMurdererSelect(murderer!.id, murderer!.meansCards[0].id, murderer!.clueCards[0].id);
      expect(result.ok).toBe(true);
    });
  ...
  describe('指证阶段 - 目击者放选项物', () => {
    ...
    it('should move marker to new board when same marker number is placed again', () => {
      ...
      const state = engine.getGameStateForPlayer(witness!.id);
      const updatedBoards = state.boards as Array<{ id: string; marker?: { optionIndex: number; markerNumber: number } }>;
  ...
  describe('场景板替换', () => {
    ...
    it('should reject replacing the death cause board', () => {
      ...
      const result = engine.handleWitnessReplaceBoard(
        witness!.id, causeBoard!.id, 'SB999', 0, 1
      );
      expect(result.ok).toBe(false);
